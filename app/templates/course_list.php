        ...
        <table class="min-w-full divide-y divide-gray-200 shadow-md rounded-lg overflow-hidden">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <?php if (!empty($courses_to_display)): ?>
                    <?php foreach ($courses_to_display as $course): ?>
                        <tr class='hover:bg-gray-50'>
                            ...
                            <td class='px-6 py-4 whitespace-nowrap text-right text-sm font-medium'>
                                ...
                                <button onclick="deleteCourse('<?php echo htmlspecialchars($course['course_code']); ?>', '<?php echo htmlspecialchars($course['course_name']); ?>')"
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan='6' class='px-6 py-4 text-sm text-gray-500 text-center'>No courses found for this user.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
