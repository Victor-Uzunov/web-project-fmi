...
<div class="p-6 bg-white rounded-lg shadow-sm">
    <h2 class="text-2xl font-semibold text-indigo-700 mb-4">All Courses Dependency Graph</h2>
    <p class="text-gray-600 mb-4">This graph shows the dependencies between all courses in the system. Click on a course node for more details. Drag nodes to rearrange the graph. Scroll to zoom.</p>
    ...
    fetch('api/courses_graph_data.php')
        ...
        .then(data => {
            ...
            if (data.nodes.length === 0) {
                if (loadingMessage) {
                    loadingMessage.style.display = 'block';
                    loadingMessage.textContent = "No courses found to display a graph.";
                }
                return;
            }
            ...
            const options = {
                ...
                groups: {
                    'Mathematics': { color: { background: '#A7F3D0', border: '#10B981' } },
                    'Software Technologies': { color: { background: '#BFDBFE', border: '#3B82F6' } },
                    'Informatics': { color: { background: '#FDE68A', border: '#F59E0B' } },
                    'Database': { color: { background: '#FBCFE8', border: '#EC4899' } },
                    'English': { color: { background: '#FED7AA', border: '#F97316' } },
                    'Soft Skills': { color: { background: '#E0E7FF', border: '#6366F1' } },
                    'Other': { color: { background: '#E5E7EB', border: '#6B7280' } }
                },
                interaction: {
                    hover: true,
                    tooltipDelay: 300,
...
function openCourseDetailsModal(node) {
    ...
    document.getElementById('modalDepartment').textContent = node.department;
    document.getElementById('modalCredits').textContent = node.credits;
